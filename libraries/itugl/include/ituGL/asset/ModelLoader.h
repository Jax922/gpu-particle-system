...
public:
    ...
    // Get Buffer Data
    inline std::vector<std::vector<float>> GetBufferDatas() {
        return m_bufferDatas;
    };
    //inline int GetTotalStride() { return m_totalStride; };
    static int TotalStride;
...
private:
    ...
    // Get the buffer data
    std::vector<std::vector<float>> m_bufferDatas;
